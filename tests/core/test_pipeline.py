...
class ConcretePipeline(Pipeline[StubPipelineStageRunner, StubPipelineStorage]):
    @staticmethod
    def init_stagerunner() -> StubPipelineStageRunner:
        return StubPipelineStageRunner()

    @staticmethod
    def init_storage() -> StubPipelineStorage:
        return StubPipelineStorage()


class TestPipeline:
    @pytest.fixture()
    def ready_made_pipeline(self) -> ConcretePipeline:
        p = ConcretePipeline("test_pipeline")
    ...
    def test_pipeline_can_be_instantiated_with_existing_stages(self, ready_made_pipeline: Pipeline):
        new_pipeline = ConcretePipeline("new_pipeline", stages=ready_made_pipeline.stages)
        for stage_name, stage in ready_made_pipeline.stages.items():
            assert new_pipeline.stages[stage_name].func is stage.func
